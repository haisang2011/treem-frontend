...
export default function CustomPaginationActionsTable({ otherCircumstances, listHCK, totalOtherCircumstances, onHandlePagination }) {

  const classes = useStyles2();
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);

  const emptyRows = rowsPerPage - Math.min(rowsPerPage, otherCircumstances.length - page * rowsPerPage);
  ...
  return (
    <div className="table-nguycohoancanhdacbiet">
    <TableContainer component={Paper} className={classes.container}>
      <Table className={classes.table}>
        ...
        <TableBody>
          {otherCircumstances.length>0 && otherCircumstances.map((row, index) => (
            <TableRow key={row.id_treem} style={(index % 2) ? { backgroundColor: "#e9e9e9" } : { backgroundColor: "white" }}>
              ...
              {row.hoancanhkhac.map(({name, value}) => (
    ...
    <div className="table-nguycohoancanhdacbiet__pagination">
      <TablePagination
        rowsPerPageOptions={[5, 10, 25, { label: 'All', value: -1 }]}
        colSpan={3}
        count={totalOtherCircumstances}
