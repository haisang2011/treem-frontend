...
export default function CustomPaginationActionsTable({ specialCircumstances, listHCDB, totalSpecialCircumstances, onHandlePagination }) {

  const classes = useStyles2();
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);

  // const emptyRows = rowsPerPage - Math.min(rowsPerPage, specialCircumstances.length - page * rowsPerPage);
  ...
  return (
        ...
        <TableBody>
          {specialCircumstances.length>0 ? specialCircumstances.map((row, index) => (
            <TableRow key={row.id_treem} style={(index % 2) ? { backgroundColor: "#e9e9e9" } : { backgroundColor: "white" }}>
              ...
              {row.hoancanhdacbiet.map(({name, value}) => (
    ...
    <div className="table-hoancanhdacbiet__pagination">
      <TablePagination
        ...
        colSpan={3}
        count={totalSpecialCircumstances}
